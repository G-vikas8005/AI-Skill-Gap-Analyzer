...
const ATSAnalysis = () => {
  ...
  const atsScore = Math.min(
    Math.max(layoutAnalysis.atsScore ?? 0, 0),
    100
  );
    ...
    <div className="min-h-screen bg-[#FFF0E4] p-6 md:p-10">
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-8">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-md">
          <h3 className="text-xl font-bold text-[#007979] mb-4">
            Resume Statistics
          </h3>
          ...
          <div className="space-y-3">
            <p>
              <strong>Pages:</strong>{" "}
              {layoutAnalysis.pages ?? "Unknown"}
            </p>

            <p>
              <strong>Layout:</strong>{" "}
              {layoutAnalysis.columns === undefined
                ? "Unknown"
                : layoutAnalysis.columns === 2
                ? "Multi Column"
                : "Single Column"}
            </p>
            ...
            <p>
              <strong>Fonts:</strong>{" "}
              {Array.isArray(layoutAnalysis.fonts) &&
              layoutAnalysis.fonts.length > 0
                ? layoutAnalysis.fonts.join(", ")
                : "N/A"}
            </p>

            <p>
              <strong>Margins:</strong>{" "}
              {layoutAnalysis.marginStatus ?? "Unknown"}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-md">
          ...
          <div className="space-y-3">
            <p>
              {layoutAnalysis.columns === undefined
                ? "ℹ️ Layout information unavailable."
                : layoutAnalysis.columns === 2
                ? "⚠️ Multiple columns detected."
                : "✅ ATS-friendly single-column layout."}
            </p>

            <p>
              {layoutAnalysis.hasTables
                ? "⚠️ Tables detected."
                : "✅ No tables detected."}
            </p>

            <p>
              {layoutAnalysis.hasImages
                ? "⚠️ Images detected."
                : "✅ No images detected."}
            </p>

            <p>
              {layoutAnalysis.hasIcons
                ? "⚠️ Icons detected."
                : "✅ No icons detected."}
            </p>

            <p>
              {layoutAnalysis.hasColors
                ? "⚠️ Multiple colors detected."
      ...
      <div className="bg-white rounded-3xl p-6 shadow-md mt-8">
        ...
        {layoutAnalysis.recommendations?.length > 0 ? (
          <ul className="list-disc pl-6 space-y-2">
            {layoutAnalysis.recommendations.map(
              (item, index) => (
